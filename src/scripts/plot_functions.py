...
def gaussian_posterior_plot_n_stars(alpha_IMF, log10_N_Ia, global_params, title, philcox, no_stars = 100):
    ...
    legend_fit = plt.legend(handles=[legend_fit], fontsize=30, shadow=True, fancybox=True, loc=2, bbox_to_anchor=(0, 0.8))
    legend_true = plt.legend(handles=[legend_true], fontsize=30, shadow=True, fancybox=True, loc=2, bbox_to_anchor=(0, 0.945))
    ...
    if philcox is not None:
        ...
        #    s = int(sigma[i])
        #    t.set(text=f'{s} $\\sigma$')
        legend_philcox = plt.legend(handles=[legend_philcox], fontsize=30, shadow=True, fancybox=True, loc=2, bbox_to_anchor=(0, 1.01))
