...
% 5 {} token are mandatory
 \abstract
  ...
  % results heading (mandatory)
   {SBI achieves a $\sim1,000\times$ speed-up compared to HMC, reducing inference runtime from $\gtrsim40$ hours to mere minutes for thousands of stars. Inference on $1,000$ stars yields precise estimates for the IMF slope ($\alpha_{\rm IMF} = -2.296 \pm 0.009$) and SN\,Ia normalization ($\log_{10}(N_{\rm Ia}) = -2.889 \pm 0.010$), deviating less than 0.04\% from the ground truth. SBI also demonstrates greater robustness to model misspecification than HMC, recovering accurate parameters even with alternate yield tables or data from a cosmological simulation.}
...
We use a NPE \citep{zeghal2022neuralposteriorestimationdifferentiable} which utilises the gradients of the generative model $\mathcal{M}$ with a Neural Spline Flow \citep[NSF;][]{durkan2019neuralsplineflows} containing 20 hidden features and 10 transformation layers for the normalizing flow. The NSF uses as invertible transformation smooth and flexible spline functions to allow efficient sampling. The expressivity of the spline allows the NDE to be capture complex distributions, while also maintaining computational tractability. The final model was selected after extensive hyperparameter tuning, varying: the architecture between NSF, Masked Autoregressive Flow \citep[MAF;][]{papamakarios2018maskedautoregressiveflowdensity}, and Mask Autoregressive 
...
We train our NPE with $10^5$ data points consisting of $n_{\rm elements}=8$ simulated with the NN emulator described in the previous section. Inputs are sampled from a uniform prior over the original $5\sigma$-range of the Gaussian priors shown in Tab. \ref{tab:priors} to provide better coverage of the parameter space. Training takes $\sim35$ minutes on an Apple M1 Max.
...
Note, the methods presented here can naturally be extended to any fast and flexible GCE model \citep[e.g.][and others]{Talbot1971,vice,omega2018}, not just \texttt{CHEMPY}.
...
If we restrict ourselves to only the two global parameters $\vec\Lambda$, our NPE achieves an APE of \variable{output/posterior_APE.txt} as shown in Fig.~\ref{fig:posterior_APE}. There we can also see, that the accuracy for an individual star of the NPE is not particularly high. However, for the global parameters $\vec\Lambda$ we can boost the accuracy by combining the inference for many stars of the same galaxy.

However, currently our NPE network is not particularly good at estimating ages from abundances alone. On average our inference for ages results in an APE of $\sim27\%$ which is slightly larger than the observational noise of $20\%$ that we add during the mock up of our data.
...
%
Using a sample of 1,000 stars we infer \variable{output/CHEMPY_TNG_sbi.txt} which deviates less than $\sim0.04\%$ from the ground truth value.
...
Looking at the joint posteriors in Figs.~\ref{fig:CHEMPY_TNG_sbi}, \ref{fig:CHEMPY_alt_sbi} and \ref{fig:CHEMPY_TNG_sim_sbi} we see that also in this case SBI recovers the true parameters with less than $3\sigma$ bias.
...
\begin{itemize}
    ...
    \item Robustness to model misspecifications: Unlike HMC, which shows significant biases when faced with discrepancies between model assumptions and data, SBI demonstrates remarkable robustness. \textcolor{blue}{Even under conditions of mismatched yield tables (should be updated)} or data generated from hydrodynamical simulations, SBI provides accurate and reliable results. This robustness ensures SBI's applicability in real-world scenarios where exact model fidelity cannot be guaranteed.
\end{itemize}
