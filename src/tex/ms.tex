...
\begin{figure*}
    ...
    \vspace{-.5cm}
    \caption{Accuracy of inferred global galactic parameters $\alpha_{IMF}$ and $\log_{10}(N_{Ia})$ as a function of number of observed stars, comparing SBI (blue line) to the inferred values using HMC (red line) as done by \cite{Philcox_2019} and the ground truth values (black dashed line). For the SBI analysis we show $1\sigma$ and $2\sigma$ contours while HMC results only show $1\sigma$ statistical uncertainities as reported in Tab.~3 of \citet{Philcox_2019} (blue/red shaded regions).}
...
\paragraph{Inference on mock data from \texttt{CHEMPY} with TNG yield set}
...
%\vspace{-1.3cm}
    ...
    \vspace{-.25cm}
    \caption{Joint posterior for global galactic parameters $\alpha_{\rm IMF}$ and $\log_{10}(\rm N_{Ia})$ of $1000$ stars. The ground truth value is shown by the black dot, the posterior mean of the SBI inference is shown with the red star and white and black ellipses show the $1-3\sigma$ contours of our inference.}
...
Using a sample of 1000 stars we infer \variable{output/CHEMPY_TNG_sbi.txt} which deviates less than $\sim0.04\%$ from the correct value.
...
\subsection{Inference on mock data from \texttt{CHEMPY} with incorrect yield set}
